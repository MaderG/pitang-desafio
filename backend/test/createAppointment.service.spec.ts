...
import { MAX_DAILY_APPOINTMENTS, MAX_HOURLY_APPOINTMENTS } from '../src/constants';
...
describe('CreateAppointmentService', () => {
  ...
  it('should throw PastDateError if date is in the past', async () => {
    ...
    await expect(service.createAppointment(inputData, dateObj)).rejects.toThrow(PastDateError);
  });
  ...
  it('should throw BookingBoundsError if daily limit is exceeded', async () => {
    ...
    (prisma.appointment.findMany as jest.Mock).mockResolvedValue(new Array(MAX_DAILY_APPOINTMENTS));

    await expect(service.createAppointment(inputData, dateObj)).rejects.toThrow(BookingBoundsError);
  });
  ...
  it('should throw BookingBoundsError if hourly limit is exceeded', async () => {
    ...
    (prisma.appointment.findMany as jest.Mock).mockImplementation(({ where }) => {
      const startHour = startOfHour(dateObj).getTime();
      const endHour = endOfHour(dateObj).getTime();

      if (where.date.gte.getTime() === startHour && where.date.lte.getTime() === endHour) {
  ...
  it('should throw AlreadyBookedError if appointment already exists', async () => {
    ...
    await expect(service.createAppointment(inputData, dateObj)).rejects.toThrow(AlreadyBookedError);
  });
});
