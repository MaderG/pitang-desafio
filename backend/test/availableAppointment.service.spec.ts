...
import { MAX_DAILY_APPOINTMENTS } from '../src/constants';
...
describe('AvailableAppointmentService', () => {
  ...
  describe('listAvailableTimes', () => {
    it('should throw an error if date is missing or invalid', async () => {
      await expect(service.listAvailableTimes('invalid-date')).rejects.toThrow(InvalidDateError);
    });
    ...
    it('should return a list of available times for a given date', async () => {
      ...
      const expectedTimes = ['08:00', '09:00', '12:00', '13:00', '14:00', '15:00', '16:00', '17:00'];

      expect(result).toEqual(expectedTimes);
      expect(prisma.appointment.findMany).toHaveBeenCalledWith({
        where: { date: { gte: startOfDay(parseISO(mockDate)), lte: endOfDay(parseISO(mockDate)) } },
        select: { date: true },
      });
    });
  });
  ...
  describe('listUnavailableDays', () => {
    it('should return a list of days that have reached the maximum number of appointments', async () => {
      const mockGroupedAppointments = [
        { date: new Date('2023-01-01T03:00:00.000Z'), _count: MAX_DAILY_APPOINTMENTS }, // GMT -03:00
        { date: new Date('2023-01-02T03:00:00.000Z'), _count: 1 },
      ];

      (prisma.appointment.groupBy as jest.Mock).mockResolvedValue(mockGroupedAppointments);
