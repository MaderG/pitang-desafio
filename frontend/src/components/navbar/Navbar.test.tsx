...
describe('<Navbar />', () => {
  ...
  it('should render the logo and navigation links', () => {
    ...
    expect(screen.getAllByText('Consultar Agendamentos')[0]).toBeInTheDocument();
  });
  ...
  it('should toggle the mobile menu when the hamburger icon is clicked', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByRole('link', { name: 'Agendar Vacina' })).toBeVisible();
      expect(screen.getByRole('link', { name: 'Consultar Agendamentos' })).toBeVisible();
    });

    fireEvent.click(hamburgerIcon);

    await waitFor(() => {
      expect(screen.queryByRole('link', { name: 'Inicio' })).not.toBeVisible();
      expect(screen.queryByRole('link', { name: 'Agendar Vacina' })).not.toBeVisible();
      expect(screen.queryByRole('link', { name: 'Consultar Agendamentos' })).not.toBeVisible();
    });
  });
  ...
  it('should navigate to the correct routes when links are clicked', () => {
    ...
    const agendarVacinaLink = screen.getByRole('link', { name: 'Agendar Vacina' });
    const consultarAgendamentosLink = screen.getByRole('link', { name: 'Consultar Agendamentos' });
