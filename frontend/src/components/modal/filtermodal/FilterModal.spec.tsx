...
describe('<FilterModal />', () => {
    ...
    it('should close the modal when the close button is clicked', async () => {
        ...
        await waitFor(() => {
            expect(
                screen.queryByText('Data de Agendamento:')
    ...
    it('should add a status when a new status checkbox is checked', () => {
        ...
        expect(mockProps.setSelectedStatuses).toHaveBeenCalledWith([
            'Pendente',
            'Cancelado',
        ])
    })
    ...
    it('should not be able to apply a date that is not in the available dates list', () => {
        fireEvent.click(screen.getByText('Filtrar agendamentos'))
        const dateInput = screen.getByTestId('input')
        fireEvent.change(dateInput, { target: { value: '2024-07-11' } })
        const applyButton = screen.getByRole('button', {
            name: 'Aplicar Filtros',
        })
        fireEvent.click(applyButton)
